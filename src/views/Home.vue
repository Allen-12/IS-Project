<template>
    <v-container fluid>
      <v-row dense>
        <v-col cols="6">
           <v-card>
            ...
            <v-card-subtitle class="text-h6 font-weight-bold">{{ availableVolume.toFixed(2) }}</v-card-subtitle>
          </v-card>
        </v-col>
        <v-col cols="6">
           <v-card>
            ...
            <v-card-subtitle class="text-h6 font-weight-bold">{{ flowRate.toFixed(2) }}</v-card-subtitle>
          </v-card>
        </v-col>
      </v-row>
      <v-row dense>
      ...
      <v-row>
        <v-col cols="12">
          <bar-chart v-if="loadedData" :volume="availableVolume" :maxVolume="maxVolume"></bar-chart>
        </v-col>
      </v-row>
    </v-container>
</template>
...
export default {
  ...
  methods: {
    ...
    setTankData(data)
    {
      ...
      this.availableVolume = parseFloat(this.surfaceArea * (this.height - this.tankWaterHeight)) / 1000

      this.timeLeft = parseFloat(this.availableVolume/this.flowRate).toFixed(0)
      let today = moment()
      this.time = today.add(this.timeLeft, 'm').format("dddd, MMMM Do YYYY, h:mm:ss a")
